...
const EventEditModal: React.FC<EventEditModalProps> = ({ show, onHide, event }) => {
  const { createEvent, updateEvent, loading } = useEvent()
  const { user } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) {
      ...
      toast.error("Please log in to create or update an event")
      return
    }
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, submit: "Failed to save event" }))
  ...
  const handleSaveAsDraft = async () => {
    ...
    try {
      if (event) {
        await updateEvent(event._id, eventData)
        toast.success("Event updated as draft successfully!")
      } else {
        await createEvent(eventData)
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setPreviewUrls((prev) => {
        if (prev.eventImage && prev.eventImage !== event?.imageUrl) {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setErrors((prev) => ({
      ...prev,
      termsCheck: e.target.checked ? "" : "Please check the box",
    }))
  }
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered" style={{ maxWidth: "800px" }}>
          <div className="modal-content" style={{ maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div
              ...
              style={{ background: "#ffffff", color: "#000000", borderBottom: "1px solid #dee2e6" }}
            >
              <h5 className="modal-title">{event ? "Edit Event" : "Create New Event"}</h5>
              <button
                type="button"
                className="btn-close btn-close-black"
                onClick={isHost ? handleCloseWithConfirmation : onHide}
                disabled={loading}
                style={{ filter: "none" }}
              ></button>
            </div>
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: "50%",
                          backgroundColor:
                            currentStep > index + 1 ? "#5144A1" : currentStep === index + 1 ? "#5144A1" : "#dee2e6",
                ...
                <div className="form-card">
                    ...
                    <div className="form-step">
                      ...
                      <div className="row align-items-end">
                        <div className="col-md-6 mb-3">
                          ...
                          {errors.guestCount && <div className="text-danger mt-1">{errors.guestCount}</div>}
                        </div>
                        <div className="col-md-6 mb-3">
                      ...
                      <div className="mb-3">
                        ...
                        <div className="input-group" style={{ height: "50px" }}>
                          ...
                          {(previewUrls.eventImage || event?.imageUrl) && (
                            <span className="input-group-text" style={{ height: "100%" }}>
                              <img
                                src={previewUrls.eventImage || event?.imageUrl || "/placeholder.svg"}
                                alt="Event preview"
                                style={{
                                  maxWidth: "60px",
                                  maxHeight: "60px",
                                ...
                                onClick={() => handleImagePreview(previewUrls.eventImage || event?.imageUrl || "")}
                              />
                            </span>
                          )}
                        </div>
                  ...
                  {currentStep === 2 && (
                    <div className="form-step">
                      ...
                      <p className="text-muted mb-4">
                        Please review all details before {event ? "updating" : "creating"} your event.
                      </p>

                      <div className="review-section mb-4">
                        {event?.isDraft && isHost && (
                          <div
                        ...
                        <div className="mb-2">
                          ...
                          <span
                            ...
                            title={formData.suggestedDonation ? `$${formData.suggestedDonation}` : "Not specified"}
                          >
                            {formData.suggestedDonation ? `$${formData.suggestedDonation}` : "Not specified"}
                          </span>
                        </div>
                        <div className="mb-2">
                ...
                <div>
                  ...
                  {currentStep < totalSteps ? (
                    ...
                    <button
                      ...
                      style={{ cursor: loading || !isTermsChecked ? "not-allowed" : "pointer" }}
                    >
                      {loading ? (event ? (event.isDraft ? "Saving..." : "Updating...") : "Creating...") : (event ? (event.isDraft ? "Save" : "Update Event") : "Create Event")}
                    </button>
                  )}
                </div>
              </div>
